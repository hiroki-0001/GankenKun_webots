...
const.moter_parameter = 18 #首のヨー軸は関係なさそうなので、抜きました
...
def file_read():
    with open("../play_motion/before_optimize_kick.csv", "r") as f:  # kick_motion.csvを読み込む
...
def select_parameter(param, motiondata):
    motiondata[6][0] = str(int(param[0])) #モーションファイルの6行0列目 = 5フレーム目の時間を指定している
    motiondata[7][0] = str(int(param[1])) #モーションファイルの7行0列目 = 6フレーム目の時間を指定している
