...
supervisor = Supervisor() #webots上でsuper visorを使用することを許可する
time_step = int(supervisor.getBasicTimeStep())

field = Field("kid") #kidサイズのフィールドを選択
...
player = supervisor.getFromDef('PLAYER')#PLAYERにsuper visorを与える
ball = supervisor.getFromDef('BALL')#BALLにsuper visorを与える
...
#ファイルの書き換えの処理
    # with()as f: でファイル操作後のcloseの処理を短縮している。
    with open("../play_motion/kick_motion.csv", "r") as f: #kick_motion.csvを読み込む 
        ...
        data = [row for row in read_data] #リスト内包表記を使用し、read_dataからデータを二次元配列として取得
        data[6][0] = str(int(param[0])) #モーションファイルの6行0列目 = 6フレーム目の時間を指定している
        data[7][0] = str(int(param[1])) #モーションファイルの7行0列目 = 7フレーム目の時間を指定している
        data[7][3] = str(int(param[2])) #モーションファイルの7行3列目 = 足先ピッチの角度を指定している
        data[7][4] = str(int(param[3])) #モーションファイルの7行4列目 = 膝ピッチの角度を指定している

    with open("../play_motion/motion.csv", "w") as f: #motion.csvに上書きする(前の状態から更新される)
        writer = csv.writer(f) # motion.csvに書き込む準備
        writer.writerows(data) # 1行ずつ書き込み
#----------------
#ループ後に条件をもとに戻すための処理
    count = 0
    player.remove()
